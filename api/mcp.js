...
// Chaque utilisateur fournit sa propre clé API iopool via l'en-tête
// "Authorization: Bearer <cle_api_iopool>" (ou "x-iopool-api-key" en repli).
// Aucune clé n'est stockée côté serveur : elle est simplement relayée
// vers l'API iopool à chaque appel.
      ...
      case "initialize": {
        res.status(200).json(
          jsonRpcResult(id, {
            ...
            capabilities: { tools: {} },
            serverInfo: { name: "iopool-mcp", version: "1.0.0" },
          })
        );
        return;
      }
              ...
              content: [
                {
                  type: "text",
                  text:
                    "Clé API iopool manquante. Configure l'en-tête 'Authorization: Bearer <ta_cle_api>' dans ton client MCP.",
                },
              ],
              isError: true,
            })
